...
//     input logic [7:0] tx_data,
//     output logic [7:0] rx_data,
//     input logic ack_in,  // resd 시 master가 보낼 ACK(0),NACK(1)
//     output logic ack_out,  // write 시 slacve 로부터 받은 ACK(0)/NACK(1)
//     output logic busy,
//     output logic done,
//     // external i2x port
//     output logic scl,
...
//             state <= IDLE;
//             scl_r <= 1'b1;  // idle SCL high
//             sda_r <= 1'b1;  // idle sda hifh (hi-z, pull_up)
//             step <= 0;
//             done <= 1'b0;
//             tx_shift_reg <= 0;
//             rx_shift_reg <= 0;
...
//                                 step <= 2'd1;
//                                 scl_r <= 1'b0;
//                                 sda_r <= is_read ? 1'b1 : tx_shift_reg[7];  // 1'b1은 하이 임피던스z를 의미
